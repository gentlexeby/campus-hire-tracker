...
- 默认“当前日程快照”是供用户手动导入的普通日历数据，不写 `METHOD`，只包含所选范围内未删除且未取消的事件。`METHOD:PUBLISH` 属于 iTIP 日程发布语义，并要求完整、稳定的组织者信息，不能拿来冒充普通文件导入。事件未出现在快照中不代表要求日历客户端删除它。
- 编辑后再次导出沿用同一 `UID`，提高 `SEQUENCE` 并刷新 `DTSTAMP`；客户端是否应用更新仍由客户端决定。
- 当前版本不生成取消通知文件。未来若提供该能力，必须先设计稳定的 `ORGANIZER` 身份和取消 tombstone，再单独生成只含取消/软删除事件的 `METHOD:CANCEL` 文件；事件沿用原 `UID`，包含更高 `SEQUENCE`、`DTSTAMP` 和 `STATUS:CANCELLED`。硬清除前必须保留完成取消导出所需的 tombstone，或明确警告清除后无法通知外部日历。
- 未来的“全量导出”若同时提供取消记录，应把普通当前快照与命名清晰的取消文件分开。由于 `METHOD` 属于整个 `VCALENDAR`，两类事件不能混在同一文件中冒充完整同步。
- 产品不维护日历订阅或客户端回执，因此不能确认外部日历已更新或删除；导出确认页必须说明这一限制。
- 下载请求必须绑定用户刚刚核对的预览快照。快照标识覆盖实际导出的标题、时间、地点、链接、描述、提醒、稳定 UID、状态与版本，并采用稳定排序；下载端重新读取数据库后若标识不一致，返回冲突并要求刷新，不得把预览后新增或变化的内容静默加入文件。该无密钥摘要只用于检测预览漂移，不是授权令牌。
- 为避免关联公司或岗位更新恰好发生在多次读取之间，下载端在组装预览字段后还要复查事件 ID 与版本集合；任何不一致都必须失败关闭。
