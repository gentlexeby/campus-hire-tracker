...
> 当前项目以个人使用和 AI 辅助全栈学习为主。`codex/resume-library-mvp` 是尚未合并到 `develop` 的当前功能分支；各文档中的 M2/M3、正式备份、PWA、AI 和服务端内容属于长期方向，不代表已经实现，也不应扩大当前切片的验收范围。
...
当前只计划本地 Windows 版本。代码可保持跨平台，但未在 CI 与真实设备验证前，不得宣称支持 macOS、Linux 或手机完整使用。当前简历切片只管理外部编辑后上传的 DOCX/PDF 文件版本和岗位关联；不包含应用内编辑或 AI 优化。未来增加服务端、多用户或云同步属于新架构阶段，不能作为简单配置开关处理。
